...
var WebObserver = function(serverObject, port)
{
    ...
    this.uriMap = {
        ...
        '^\\/?$' : function(request, response) {self.set302(response, '/world.html'); return true;},
        '^\\/.+$' : this.processStatic
    };
};
...
WebObserver.prototype.processRequest = function(request, response)
{
    ...
    for(regexp in this.uriMap) {
        if (this.uriMap.hasOwnProperty(regexp)) {
            if (uri.match(regexp)) {
...
WebObserver.prototype.run = function()
{
    ...
    util.log('WebObserver: http://127.0.0.1:' + this.port);
};
